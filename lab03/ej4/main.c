...
int main(int argc, char *argv[]) {
    ...
    int temp_min = hist_temp_min(array);

    printf("\nThe historical minimum temperature in Cordoba is %d\n", temp_min);

    int outpout[YEARS];

    max_temp_of_year(array, outpout);

    printf("\n");
    for (unsigned int year=1980; year<=2016; year++){
        printf("%d ", outpout[year]);
    }

    month_t month[YEARS];

    max_rainfall_mounth(array, month);
